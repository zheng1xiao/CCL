...
def evaluate(X_test, Y_test, model1):
    ...
    return acc_1, f1_1, mcc_1, prec1, recall1, roc_auc1, effort1


def train(train_loader, Y_train, epoch, model1, optimizer1):
    ...
    for i, (x_data, labels, indexes) in enumerate(train_loader):
        ...
        loss_sum = F.cross_entropy(predict_p_1, s_pruned, reduction='none')
...
def main():
    ...
    file_name = 'ORI-LR.csv'
    f = open(file_name, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(f)
    csv_writer.writerow(
        ["datasets", "acc_1", "mcc_1", "f1_1", "prec1", "recall1", "auc1", "effort1"])
        ...
        for ix in range(10):
            ...
            for train_index, test_index in sfolder.split(ob, label):
                ...
                for epoch in range(1, args.n_epoch):
                    ...
                    train_acc1 = train(train_loader, Y_train, epoch, model1,
                                                   optimizer1)

                model1.eval()

                test_acc1, test_f11, test_mcc1, prec1, recall1, roc1, effort_1 = evaluate(X_test, Y_test, model1)
                print('%s Epoch [%d/%d] Test Accuracy on the %s test: Model1 %.4f,the mcc on the test: %.4f, f1 : %.4f,%.4f,%.4f' % (
                   csv_string, epoch + 1, args.n_epoch, len(Y_test), test_acc1, test_mcc1, test_f11, roc1, effort_1))
