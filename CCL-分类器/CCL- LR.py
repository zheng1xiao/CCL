...
def evaluate(X_test, Y_test, model1, model2):
    ...
    mask1 = out_1.ge(0.5).float()
    predict_p1 = torch.hstack((1 - out_1, out_1))
    correct1 = (mask1.cpu() == Y_test).sum()

    out2 = model2(X_test)
    out_2 = out2.squeeze()
    mask2 = out_2.ge(0.5).float()
    predict_p2 = torch.hstack((1 - out_2, out_2))
    correct2 = (mask2.cpu() == Y_test).sum()

    loc_all = sum(X_test[:, 4]) + sum(X_test[:, 5])
    loc_20 = loc_all * 0.2
    r_pre1 = np.zeros((len(mask1), 3))
    r_pre2 = np.zeros((len(mask2), 3))
    count_t1 = 0
    count_t2 = 0

    for il in range(len(mask1)):
        r_pre1[il][0] = (X_test[il][4] + X_test[il][5])
        r_pre1[il][1] = mask1[il]
    ...
    for i2 in range(len(mask2)):
        r_pre2[i2][0] = (X_test[i2][4] + X_test[i2][5])
        r_pre2[i2][1] = mask2[i2]
    ...
    f1_1 = metrics.f1_score(Y_test, mask1, pos_label=1, average="binary")
    f1_2 = metrics.f1_score(Y_test, mask2, pos_label=1, average="binary")
    acc_1 = accuracy_score(Y_test, mask1)
    acc_2 = accuracy_score(Y_test, mask2)
    mcc_1 = matthews_corrcoef(Y_test, mask1)
    mcc_2 = matthews_corrcoef(Y_test, mask2)
    prec1 = metrics.precision_score(Y_test, mask1, pos_label=1)  # 精确率
    prec2 = metrics.precision_score(Y_test, mask2, pos_label=1)  # 精确率
    recall1 = metrics.recall_score(Y_test, mask1, pos_label=1)  # 召回率
    recall2 = metrics.recall_score(Y_test, mask2, pos_label=1)  # 召回率
    fpr1, tpr1, thersholds1 = metrics.roc_curve(Y_test, mask1)
    roc_auc1 = metrics.auc(fpr1, tpr1)
    fpr2, tpr2, thersholds2 = metrics.roc_curve(Y_test, mask2)
...
def train(train_loader, Y_train, epoch, model1, optimizer1, model2, optimizer2, pre_new):
    ...
    for i, (x_data, labels, indexes) in enumerate(train_loader):
        ...
        out_1 = model1(x_pruned)
        mask_1 = out_1.ge(0.5).float().ravel()
        correct_1 = (mask_1.cpu() == s_pruned).sum()
        correct_all1 += correct_1
        train_total1 += x_pruned.size(0)
        predict_p_1 = torch.hstack((1 - out_1, out_1))

        out_2 = model2(x_pruned)
        mask_2 = out_2.ge(0.5).float().ravel()
        correct_2 = (mask_2.cpu() == s_pruned).sum()
        correct_all2 += correct_2
        train_total2 += x_pruned.size(0)
        predict_p_2 = torch.hstack((1 - out_2, out_2))

        criterion = nn.BCELoss(reduction = 'none')
        s_pruned = s_pruned.ravel()
        loss_1, loss_2, = loss_coteaching(predict_p_1, predict_p_2, s_pruned, rate_schedule[epoch], criterion)
        ...
        prec1 = accuracy_score(s_pruned, mask_1)
        prec2 = accuracy_score(s_pruned, mask_2)
        mcc1 = matthews_corrcoef(s_pruned, mask_1)
        mcc2 = matthews_corrcoef(s_pruned, mask_2)
        f1_1 = metrics.f1_score(s_pruned, mask_1, pos_label=1, average="binary")
        f1_2 = metrics.f1_score(s_pruned, mask_2, pos_label=1, average="binary")
        # if i == 0:
        #     # print(out_1[0:30])
        #     # print(mask_1[0:100])
