...
def evaluate(X_test, Y_test, model1, model2):
    ...
    loc_20 = loc_all * 0.2
    r_pre1 = np.zeros((len(mask), 3))
    r_pre2 = np.zeros((len(mask), 3))
    count_t1 = 0
    count_t2 = 0
    ...
    f1_1 = metrics.f1_score(Y_test, mask, pos_label=1, average="binary")
    acc_1 = accuracy_score(Y_test, mask)
    prec1 = metrics.precision_score(Y_test, mask, pos_label=1)  # 精确率
    recall1 = metrics.recall_score(Y_test, mask, pos_label=1)  # 召回率
    fpr1, tpr1, thersholds1 = metrics.roc_curve(Y_test, mask)
