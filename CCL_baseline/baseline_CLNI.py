...
def con_learn():
    file_name = 'CLNI06185.csv'
    f = open(file_name, 'w', encoding='utf-8', newline='')
    csv_writer = csv.writer(f)
    csv_writer.writerow(["datasets", "f1-ori", "f1-cl", "f1-cli", "auc-ori", "auc-cl", "auc-cli", "prec-ori", "prec-cl",
                         "prec-cli","recall-ori", "recall-cl",
                         "recall-cli", "mcc-ori", "mcc-cl", "mcc-cli","effort-ori", "effort-cl", "effort-cli"])
    #第一列是原始f值，第二列是运用置信学习去噪的结果，第三列是不平衡+置信学习
    for i in range(1):
        csv_string = csv_order[i+5]
        ...
        for ix in range(10):
            ...
            for train_index, test_index in sfolder.split(ob, label):
                ...
                y_original, thresholds1, prec1,recall1, mcc1,eff1 = get_noise(y_resampled, psx, X_resampled, X_test, y_test,seed[ix])
            ...
            effort_all2.append(np.mean(effort_or2))
        ...
        ori_all.append(np.mean(effort_all2))
